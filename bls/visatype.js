...
(function() {
        ...
        function resolveIds(arrays, choices) {
            ...
            if (!loc) {
                warn("location not found", locName);
                return res;
            }
            ...
            if (!vs) {
                warn("visaType not found", vsName);
                return res;
            }
            ...
            if (!vsSub) {
                warn("visaSubType not found", vsSubName);
                return res;
            }
            ...
            if (!cat) {
                warn("category not found", catName);
                return res;
            }
        ...
        function forceValueIntoField(id, value) {
            ...
            if (!el) {
                warn("forceValueIntoField: not found", id);
                return;
            }
        ...
        async function applyMembersField(form) {
            ...
            if (!applicants.length) {
                warn("[VT] applicantsNoData empty");
                return;
            }
            ...
            if (!value) {
                warn("[VT] applicantsNoData match has no Id/Value");
                return;
            }

            const membersId = findMembersFieldInput(form);
            if (!membersId) {
                warn("[VT] members field id not found");
                return;
            }

            forceValueIntoField(membersId, value);
            log("[VT] members injected:", {
                membersId,
                value
            });
        }
            ...
            setTimeout(async () => {
                    ...
                    try {
                        ...
                        if (resp.status === 200) {
                            ...
                            const locUpper = (window.__SAMURAI_STORAGE?.calendria_location_name || "")
                                .toString()
                                .trim()
                                .toUpperCase();
